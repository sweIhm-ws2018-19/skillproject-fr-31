...
public class Setup {
    ...
    public static Optional<Response> setupState(HandlerInput input) {
        ...
        if (persistentAttributes.get("NAME") == null) {
            // store in database
            BasicUtils.setSessionAttributes(attributesManager,GuideStates.getStateString(), GuideStates.INSERT_NAME);
            ...
            if (persistentAttributes.get("HOME") == null) {
                ...
                if (permissions == null) {
                    ...
                    BasicUtils.setSessionAttributes(attributesManager,GuideStates.getStateString(), GuideStates.GET_HOME_ADDR);
            ...
            if (persistentAttributes.get("DEST") == null) {
                BasicUtils.setSessionAttributes(attributesManager,GuideStates.getStateString(), GuideStates.GET_DEST_ADDR);
                return BasicUtils.putTogether("Ziel Adresse", SpeechStrings.START_CONFIG_DEST_ADDRESS).build();
            }

            BasicUtils.setSessionAttributes(attributesManager,GuideStates.getStateString(), GuideStates.TRANSIT);
